...
class RestartService(ManifestControl):
    ...
    def __init__(self, threadMgr, service):
        """ Constructor """
        ManifestControl.__init__(self, threadMgr, service, manifest = None, name = 'restart_service')
        self.setName(RestartService.THREAD_NAME)
    ...
    def doRun(self):
        ...
        try:
            ...
            self.__LOG.info('Done: restarted service for (%s/%s)' % (self._service, activeManifest))
            ...
            if failed:
                ...
                if self._service and activeManifest:
                    try:
                        self.__LOG.info('Restart service %s failed, shutdown to cleanup' % self._service)
                        self.__shutdownManifest(self._service, activeManifest)
    ...
    def __shutdownManifest(self, service, manifest):
        ...
        self._execPackages('shutdown', service, manifest, 11, 50, activateFlow = False)
    ...
    def __startupManifest(self, service, manifest):
        ...
        self._execPackages('startup', service, manifest, 51, 80, activateFlow = False)
